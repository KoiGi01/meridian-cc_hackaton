...
- `src/pointto.manifest.json` — **added by us.** The hand-written manifest from Checkpoint 2, kept for comparison. No longer used.
- `src/pointto.generated.manifest.json` — **generated by `pointto-cli scan`** (Checkpoint 5). This is what the app runs on now. 51 elements across 7 routes plus a shared `*` route, labeled by Gemini. Nobody typed it.
- `guide.config.json` — **added by us.** The scanner's input: the routes to scan and where to write. No secrets; the Gemini key comes from the environment.
